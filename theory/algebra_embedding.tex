...
S(Q,e) = -\log Q(e)
\end{align}
With the logarithmic rule, inference based on scoring rule minimization is just (depending on the context) maximum likelihood, maximum conditional likelihood, or minimum cross-entropy. Here we are interested in discriminative training and therefore in maximum conditional likelihood (MCL), which we will discuss in the next section. Unfortunately, for our training problem, MCL is intractable, so we need to find alternatives, which we will construct using scoring rules other than the logarithmic one.

The other way to define proper scoring rules works like this~\cite{DeGroot}. Let $Q$ be a probability distribution to predict an event $e\in\Eset$. Let $d\in\Dset$, be a \emph{decision} that is made given $Q$ (while $e$ is still unknown). Let $C(d,e)$ be a real-valued function, known as \emph{cost function} that quantifies the goodness of decision $d$, when afterwards it becomes known that the value of the event is $e$. Let $d^*_Q\in\Dset$ be a minimum-expected-cost \emph{Bayes decision}, with the property:
\begin{align}
\expv{C(d^*,e)}{e\sim Q} \le \expv{C(d,e)}{e\sim Q}
\end{align} 
for any $d\in\Dset$. Then 
\begin{align}
\label{eq:psr2}
S(Q,e) &= C(d^*_Q,e)
\end{align}
...
Why are proper scoring rules good for training? 


\section{The CRP partition prior}
\def\Li{\Lset_{\setminus i}}
\def\Iset{\mathcal{I}}
In this chapter we are concerned with probability distributions over all possible ways to partition a set of $n$ recordings. Specifically, for discriminative training, we need to work with posterior distributions. To compute posteriors, we first need a prior. 

A convenient prior over partitions can be constructed using the two-parameter (Pitman-Yor) \emph{Chinese restaurant process} (CRP)~\cite{Goldwater,Pitman}. The CRP gives a recursive construction for a probability distribution over partitions of a set of $n$ recordings, as follows. Let $\Lset:\Sset_1,\Sset_2,\ldots,\Sset_k$ be a $k$-speaker partition of the recordings, expressed, as before, in terms of index subsets. The CRP gives:  
\begin{align}
P(\Lset) &= P(\ell_i\mid \Li)P(\Li), \;\;\;\;\;\forall i\in\{1,\ldots,n\}
\end{align}
where the choice of $i$ is arbitrary and where $\Li:\Sset'_1,\Sset'_2,\ldots,\Sset'_{k'}$ is the partition that results from removing recording $i$ from $\Lset$. That is, $\Li$ is obtained from $\Lset$ by removing $i$ from the subset where it occurs. If the modified subset remains non-empty, then $k'=k$, else that subset is removed, so that $k'=k-1$. The categorical variable, $\ell_i\in\{1,2,\ldots,k',k'+1\}$ denotes the subset (speaker) to which $i$ belongs. If $\ell_i=k'+1$, then a new subset (speaker) is implied. This definition is recursive, so that $P(\Li)$ is similarly defined, where the recursion ends when $\Lset_i$ is the trivial partition of a set with one element. To compute the probability for a given $\Lset$, the recursion can be done from large to small, or from small to large. The small to large direction can also be used as a generative method to sample from this prior.

For $\Li$ of size $m$, the recursion step is defined as:
\begin{align}
P(\ell_i=j\mid \Li) &= \begin{cases}
\frac{\lvert\Sset'_j\rvert-\beta}{m+\alpha}, &\text{for $1\le j\le k'$}\\
\frac{k'\beta+\alpha}{m+\alpha}, &\text{for $j=k'+1$}
\end{cases}
\end{align}
where $\lvert\Sset'_j\rvert$ is the number of elements in the subset. The CRP is parametrized by $\alpha\ge0$ (concentration) and $0\le\beta<1$ (discount).\footnote{The model can be extended to allow some negative values for the parameters. See \url{http://en.wikipedia.org/wiki/Chinese_restaurant_process}, or~\cite{PitmanBook}.} The expected number of speakers is:
\begin{align}
\label{eq:CRP_ev}
\expp{k}{} &= \begin{cases}
1& \text{for $\alpha=\beta=0$},\\
\alpha(\psi(n+\alpha)-\psi(\alpha))&   \text{for $\alpha>0, \beta=0$}, \\
\frac{\Gamma(\alpha+\beta+n)\Gamma(\alpha+1)}{\beta\Gamma(\alpha+n)\Gamma(\alpha+\beta)}-\frac{\alpha}{\beta}&   \text{for $\alpha>0, \beta>0$}
\end{cases}
\end{align}
where $\Gamma$ and $\psi$ the gamma and digamma functions. Notice that $1\le\expp{k}<n$, where the upper limit is reached at $\alpha\to\infty$, or $\beta\to1$. 

\subsection{Example}
We let the number of recordings be $n=20$. For each of four different choices of the CRP parameters, $\alpha,\beta$, we draw $1000$ random partitions. For each of the four cases, we draw a histogram of $k$, the number of speakers:
$$
...
\section{The partition posterior}
...
where $\theta$ are the model parameters. For discriminative training, the natural criterion is \emph{maximum conditional likelihood}\footnote{Why is $P(\text{labels}\mid\text{data})$ termed conditional, while $P(\text{data}\mid\text{labels})$ is not? In the former case, $P(data)$ is not used: everything is conditional on the data, which is given. In the latter case, $P(\text{labels})$ could be used, but often $P(\text{labels})$ is so trivial that it is ignored.} (MCL):
...
In our case, MCL is given by:
\begin{align}
\begin{split}
&\argmax_\theta P(h^*\mid\Rset,\theta)\\
=&\argmax_\theta \frac{P(h^*) P(\Rset\mid h^*,\theta)}{\sum_h P(h) P(\Rset\mid h,\theta)}
\end{split}
\end{align}
Where $\Rset=\{r_1,\ldots,r_n\}$ is a set of recordings and $h^*$ is a single `label', namely the true partitioning of $\Rset$ w.r.t.\ speaker. The summation in the denominator is over all possible ways to partition a set of $n$ elements, where $h$ is one such partition.\footnote{Do not confuse partition with subset. For $n$ recordings, a
partition is a collection of non-overlapping, non-empty index subsets, whose union is $\{1,\ldots,n\}$.} We have also introduced $P(h)$, a prior probability distribution over partitions. By defining the likelihood-ratio:
\begin{align}
L(h,h^*;\theta) &= \frac{P(\Rset\mid h,\theta)}{P(\Rset\mid h^*,\theta)}
\end{align}
We can rewrite the label posterior as:
\begin{align}
P(h^*\mid\Rset,\theta) &= \frac{P(h^*)}{\sum_h P(h) L(h,h^*;\theta)}
\end{align}
If we are given a meta-embedding for every $r_j$, and if arbitrary pooling and expectations (or inner products) are tractable, then we also have that $L(h,h^*;\theta)$ is tractable for any $h,h^*$. The problem is that the number of terms in the denominator (the Bell number) is hopelessly intractable, except for very small $n$. A speaker recognizer is typically trained with $n$ in the thousands, but for $n=75$, we already have that the Bell number exceeds $10^{80}$, the estimated number of atoms in the observable universe!

\subsection{Is one label enough?}
The MCL objective is very different from the familiar cross-entropy objective, in the sense that it has but a single label, while cross-entropy is usually applied to a training database with very many independent, labelled examples. One answer is that this single label does contain many bits of information. Using an approximate expression for the log Bell number,\footnote{See: \url{http://en.wikipedia.org/wiki/Bell_number}.} the number of bits we need to uniquely specify a partition of $n$ recordings is about:
$$
...
\cite{Dawid_Musio_ThApp_PSR_2014,Dawid_discrete_PSR,Besag_pseudolikelihood}
