...
\chapter{Meta-embeddings derived from generative models}
...
\end{align}
we can use~\eqref{eq:Dirac_scoring} to approximate the log-likelihood for each class $i$ as:
\begin{align}
\begin{split}
\log L_{ij} &\approx \log g_j(\hat\zvec_i) - \log\expp{g_j} \\
&= \avec_j'\zvec_i -\frac{\beta_j}{2}\hat\zvec_i'\Bmat\hat\zvec_i -\frac{1}{2}\avec_j(\Imat+\beta_j\Bmat)^{-1}\avec_j -\frac12\log\detm{\Bmat} -\frac{d}{2}\log{\beta_j}
\end{split}
\end{align}
For the open-set case, we can omit the constant $\frac12\log\detm{\Bmat}$, while for the \emph{closed-set} case, we can omit all terms independent of $i$, to give:
\begin{align}
...
As a sanity check, let's derive this scoring recipe directly from~\eqref{eq:LRMVG}:
\begin{align}
\lim_{\beta_i\to\infty} \log E(\avec_i+\avec_j,(\beta_i+\beta_j)\Bmat) - \log E(\avec_j,\beta_i\Bmat)
\end{align}


\subsubsection{Doing it without Gaussians}
As an alternative (perhaps a good idea when $D-d$ is small), both training and scoring can be done without Gaussian approximations. For training, the class representatives, $\hat\zvec_i$, can be found by numerical maximization of the product of t-distributions. If in doubt about the sharpness of the peaks at the maxima, we could verify this by using the Laplace approximation as explained in section~\ref{sec:Laplace}. If all peaks are indeed sharp, we can score closed-set multiclass likelihoods by plugging the $\hat\zvec_i$ into the t-distribution meta-embeddings of the test segments, as in~\eqref{eq:Dirac_scoring}.
