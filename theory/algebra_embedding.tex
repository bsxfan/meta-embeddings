...
\end{enumerate}
These two requirements are met by the configuration $\zvec\to\rvec_j\gets\svec_j$. In full graphical model notation, the model (for a single speaker) can be specified as below:%specification is given in figure~\ref{fig:aug_model}. 
...
\end{align}
%} %centerline
%\caption{Generative model for augmented i-vectors of a single speaker.}
%\label{fig:aug_model}
%\end{figure}
...
\section{Non-linear PLDA}
For a speaker with identity variable, $\zvec\in\R^d$, let recording $r_j\in\R^D$ be generated as follows:
\begin{align}
P(\xvec_j\mid\zvec) &= \ND(\xvec_j\mid\Fmat\zvec,\Wmat^{-1}), &
\rvec_j &= f(\xvec_i)
\end{align}
where $f$ is an invertible non-linearity, i.e.\ a bijection from $\R^D$ to $\R^D$. The model parameters are the usual SPLDA parameters, $\Fmat,\Wmat$, as well as some parameters (not shown here) that make the non-linearity trainable. Let $J(\xvec)$ denote the absolute value of the Jacobian determinant of $f$, where the Jacobian is the matrix of derivatives of $\rvec$ w.r.t.\ $\xvec$. Then we have:
\begin{align}
P(\rvec_j\mid\zvec) &= \frac{P(\xvec_j\mid\zvec)}{J(\xvec_j)}, &
\xvec_j &= f^{-1}(\rvec_j)
\end{align}
Since $J(\xvec)$ is independent of $\zvec$, we don't need it to extract meta-embeddings. The meta-embedding, $P(\rvec_j\mid\zvec) \propto P(\xvec_j\mid\zvec)$ is Gaussian and identical in form to that extracted by ordinary SPLDA. At scoring time, $f^{-1}$ functions as a feature extractor, which can be followed by the ordinary SPLDA scoring recipe.

\subsection{Discriminative training}
Discriminative training criteria that depend on the model parameters only through scoring, need to use neither $f$, nor $J$. We only need to use $f^{-1}$ at training time. We can therefore directly train some feature extractor, $\rvec_j\to\yvec_j$, jointly with the PLDA parameters.

\subsection{Direct generative training}
For an observed data set, $\Yset$, with labels, $\Lset$, the SPLDA model evidence $P(\Yset\mid,\Fmat,\Wmat,\Lset)$ can 
...
\begin{figure}[htb!]
\centering
\includegraphics[height=6cm,trim={3cm 8cm 4cm 8cm},clip]{SGME_nu_experiment_a.pdf}        %trim parameters: left, bottom, right, top
\includegraphics[height=6cm,trim={3cm 8cm 4cm 8cm},clip]{SGME_nu_experiment_b.pdf}        %trim parameters: left, bottom, right, top
...
\end{itemize}
The values of $\Hmat,\Pmat$ and $\Lambdamat$ were found with a full batch L-BFGS optimizer. In this experiment (and in the larger one below) L-BFGS, running on a laptop, converged in a few minutes.
...
\end{tabular} 
$$
The accuracy\footnote{{The plots and the above table were for different experimental runs, with different model parameters that generated the synthetic data. The plot and table therefore show different accuracies.}} of likelihood-ratios computed for all pairs of recordings for the generative and discriminative systems are further analyzed in figure~\ref{fig:SGME_PsL_Train}, using the well-known speaker recognition tools of DET-plots~\cite{DETPLOT} and Bayes-error-rate plots~\cite{PhD,BosarisToolkit}. 


\begin{figure}[htb!]
\centering
\includegraphics[height=6cm,trim={3cm 7cm 4cm 8cm},clip]{SGME_training_DET_plot.pdf}        %trim parameters: left, bottom, right, top
...
\subsection{Using binary cross-entropy}
...
\end{tabular} 
$$
Bayes-error-rate plots are given in figure~\ref{fig:SGME_BXE_Train_DCF} and the DET plot in figure~\ref{fig:SGME_BXE_Train_DET}.


\begin{figure}[htb!]
\centering
%trim parameters: left, bottom, right, top
\includegraphics[height=5cm,trim={0cm 7cm 1.5cm 8cm},clip]{SGME__BXE_training_DCF_plot_Train.pdf}        
\includegraphics[height=5cm,trim={0 7cm 1.5cm 8cm},clip]{SGME__BXE_training_DCF_plot.pdf}
\caption[Binary cross-entropy SGME training]{Binary cross-entropy SGME training. Bayes error-rate plot for both systems on the training data (left) and test data (right).}
\label{fig:SGME_BXE_Train_DCF}
\end{figure}
...
JPLDA uses a factorial hidden variable prior, where all speaker and language factors (the $\zvec_i$ and the $\ellvec_j$) are all independent. 

\subsection{Note on notation}
In JPLDA, we shall be working with model configurations where every observation, $\rvec_k$, is dependent on exactly one hidden speaker factor and exactly one hidden language factor. (The converse is not true---multiple observations may depend on a given hidden factor.) In graphical model notation, each $\rvec_k$ has exactly one $\zvec$ parent and exactly one $\ellvec$ parent. (Conversely, hidden factors may have multiple children.) 

In hidden-variable likelihoods and posteriors, conditioned on a single observation, of the forms $P(\zvec,\ellvec\mid\rvec_k)$, $P(\zvec\mid\rvec_k)$, $P(\rvec_k\mid\zvec,\ellvec)$, $P(\rvec_k\mid\ellvec)$, etc., it is clear which $\zvec$  and $\ellvec$ variables are being referred to, namely the parents of $\rvec_k$. Indeed, notice that when no other observations are given, then dependency is blocked between $\rvec_k$ and all other hidden variables, except its parents.

Finally, irrespective of the model configuration, $P(\rvec_k)$, for any single observation is unambiguously defined as:
\begin{align}
P(\rvec_k) &= \int\int P(\zvec)P(\ellvec) P(\rvec\mid\zvec,\ellvec)\,d\zvec\,d\ellvec
\end{align}
where we have used the factorial prior, $P(\ellvec)P(\zvec)$.

\subsection{Bivariate Gaussian meta-embedding}
For JPLDA, the \emph{bivariate} analogue of our meta-embedding is a Gaussian likelihood function of the form:
\begin{align}
\label{eq:jplda_lh}
\begin{split}
f_k(\zvec,\ellvec) &\propto P(\rvec_k\mid\zvec,\ellvec) \\
&\propto \exp\Bigl(
...
\subsection{Normalization and marginalization}
Normalization of univariate and multivariate meta-embeddings will play an important role in what follows. With two kinds of hidden variables, the discrete hypothesis space (which observables are dependent on which hidden variables) is more complex---and joint normalization of all hypotheses of interest requires keeping careful track of normalization constants.

Having clarified that when conditioning on a single observation, the relevant hidden variables are its two parents, we can express Bayes' rule as:
\begin{align}
P(\zvec,\ellvec\mid\rvec_k) &= \frac{P(\ellvec)P(\zvec)P(\rvec_k\mid\zvec,\ellvec)}{P(\rvec_k)}
\end{align}
In a bivariate generalization of our previous definition of normalization, we can now define the \emph{normalized bivariate meta-embedding} as:
\begin{align}
\label{eq:norm_fk}
\normal{f_k}(\zvec,\ellvec) &= \frac{P(\zvec,\ellvec\mid\rvec_k)}{P(\ellvec)P(\zvec)} 
= \frac{P(\rvec_k\mid\zvec,\ellvec)}{P(\rvec_k)}
= \frac{f_k(\zvec,\ellvec)}{\expv{f_k(\zvec',\ellvec')}{\zvec',\ellvec'}}
\end{align}
Below, we shall also need \emph{marginal meta-embeddings}:
\begin{align}
g_k(\zvec) &\propto P(\rvec_k\mid\zvec) &&\text{and} &
h_k(\ellvec) &\propto P(\rvec_k\mid\ellvec)
\end{align}
The normalized versions are:
\begin{align}
\label{eq:norm_gk}
\normal{g_k}(\zvec) &= \frac{P(\zvec\mid\rvec_k)}{P(\zvec)} 
= \frac{P(\rvec_k\mid\zvec)}{P(\rvec_k)}
= \frac{g_k(\zvec)}{\expv{g_k(\zvec')}{\zvec'}}
\intertext{and}
\label{eq:norm_hk}
\normal{h_k}(\ellvec) &= \frac{P(\ellvec\mid\rvec_k)}{P(\ellvec)} 
= \frac{P(\rvec_k\mid\ellvec)}{P(\rvec_k)}
= \frac{h_k(\ellvec)}{\expv{h_k(\ellvec')}{\ellvec'}}
\end{align}
It follows immediately from these definitions that the normalized marginals can be recovered via partial expectations of the normalized bivariate meta-embedding:
\begin{align}
\normal{g_k}(\zvec) &= \expv{\normal{f_k}(\zvec,\ellvec)}{\ellvec}
&&\text{and} &
\normal{h_k}(\ellvec) &= \expv{\normal{f_k}(\zvec,\ellvec)}{\zvec}
\end{align}
where the expectations are w.r.t. $P(\ellvec)$ and $P(\zvec)$, respectively.



\subsection{Gaussian marginalization}
For Gaussian JPLDA, with likelihoods of the form~\eqref{eq:jplda_lh} and independent standard normal priors on the hidden variables, we can work out the details of the meta-embedding marginalization in closed form. To avoid clutter, we do the marginalization in un-normalized form and then recover the normalization constant afterwards:
\begin{align}
\begin{split}
g_k(\zvec) &\propto \int f_k(\zvec,\ellvec) \ND(\ellvec\mid\nulvec,\Imat)\,d\ellvec \\
&\propto \int \exp\Bigl(
\bmat{\zvec'& \ellvec'}\bmat{\Vmat' \\ \Lmat'}\Wmat\rvec_k - \frac12\bmat{\zvec'& \ellvec'}
...
\end{split}
\end{align} 
where the above equations implicitly define natural parameters $\avec,\bvec$, precision blocks $\Amat,\Bmat,\Dmat$ and covariance blocks $\Qmat,\Rmat,\Smat$. Block matrix inversion gives:
\begin{align}
\Qmat^{-1} &= \Amat-\Bmat\Dmat^{-1}\Bmat' &&\text{and}&
\Rmat &= -\Qmat\Bmat\Dmat^{-1}
\end{align}
so that
\begin{align}
g_k(\zvec) &\propto \exp\Bigl(
\zvec'(\avec-\Bmat\Dmat^{-1}\bvec) -\frac12\zvec'\Qmat^{-1}\zvec
\Bigr)
\end{align}
The normalization constant can be recovered using~\eqref{eq:GMElogex}.

By symmetry, the other marginal, $h_k(\ellvec)$, can be found using the same formula, but exchanging the roles of $\Vmat$ and $\Lmat$. 
\begin{align}
\begin{split}
h_k(\zvec) &\propto \int f_k(\zvec,\ellvec) \ND(\zvec\mid\nulvec,\Imat)\,d\zvec \\
&\propto \int \exp\Bigl(
\bmat{\ellvec'& \zvec'}\bmat{\Lmat' \\ \Vmat'}\Wmat\rvec_k - \frac12\bmat{\ellvec'& \zvec'}
\bmat{\Lmat'\Wmat\Lmat & \Lmat'\Wmat\Vmat \\ \Vmat'\Wmat\Lmat & \Imat+\Vmat'\Wmat\Vmat}
\bmat{\ellvec\\ \zvec} 
\Bigr) \,d\zvec \\
%
&= \int \exp\Bigl(
\bmat{\ellvec'& \zvec'}\bmat{\bvec \\ \avec} - \frac12\bmat{\ellvec'& \zvec'}
\bmat{\tilde\Amat & \Bmat' \\ \Bmat & \tilde\Dmat}
\bmat{\ellvec\\ \zvec} 
\Bigr) \,d\zvec \\
%
&= \int \exp\Bigl(
\bmat{\ellvec'& \zvec'}\bmat{\bvec \\ \avec} - \frac12\bmat{\ellvec'& \zvec'}
\bmat{\tilde\Qmat & \tilde\Rmat \\ \tilde\Rmat' & \tilde\Smat}^{-1}
\bmat{\ellvec\\ \zvec} 
\Bigr) \,d\zvec \\
%
%
&\propto \int \ND\Bigl(\bmat{\ellvec \\ \zvec}\mid \bmat{\tilde\Qmat\bvec+\tilde\Rmat\avec\\\tilde\Rmat'\bvec+\tilde\Smat\avec}, \bmat{\tilde\Qmat & \tilde\Rmat \\ \tilde\Rmat' & \tilde\Smat}\Bigr)
 \,d\zvec \\
&=\ND(\ellvec\mid \tilde\Qmat\bvec+\tilde\Rmat\avec, \tilde\Qmat)
\end{split}
\end{align} 
Block matrix inversion gives:
\begin{align}
\tilde\Qmat^{-1} &= \tilde\Amat-\Bmat'\tilde\Dmat^{-1}\Bmat &&\text{and}&
\tilde\Rmat &= -\tilde\Qmat\Bmat'\tilde\Dmat^{-1}
\end{align}
so that
\begin{align}
h_k(\ellvec) &\propto \exp\Bigl(
\ellvec'(\bvec-\Bmat'\tilde\Dmat^{-1}\avec) -\frac12\ellvec'\tilde\Qmat^{-1}\ellvec
\Bigr)
\end{align}
...
\begin{enumerate}
	...
	\item[$H_{12}$:] they share $1$ speaker factor and there are $2$ independently sampled language factors; 
	\item[$H_{21}$:] there are $2$ independently sampled speaker factors and they they share $1$ language factor;
	\item[$H_{22}$:] they share nothing and there are $2$ of each kind of factor, all independent.
\end{enumerate}
These hypotheses have the following graphical model representations:
\begin{align*}
...
\end{tikzpicture}
\end{align*}
The likelihoods for the four hypotheses are:
\begin{align*}
...
\end{align*}
Finally, we can express all of these likelihoods in terms of normalized meta-embeddings, by conveniently dividing all likelihoods by $P(\rvec_i)P(\rvec_j)$. Using the penultimate right-hand sides of~\eqref{eq:norm_fk}, \eqref{eq:norm_gk} and~\eqref{eq:norm_hk}, this gives the mutually compatible hypothesis likelihoods:
\begin{align}
\begin{split}
P(\rvec_i,\rvec_j\mid H_{11}) &\propto \expv{\normal{f_i}(\zvec,\ellvec)\times \normal{f_j}(\zvec,\ellvec)}{\zvec,\ellvec}\\
P(\rvec_i,\rvec_j\mid H_{12}) &\propto \expv{\normal{h_i}(\ellvec)\times \normal{h_j}(\ellvec)}{\ellvec}\\
P(\rvec_i,\rvec_j\mid H_{21}) &\propto \expv{\normal{g_i}(\zvec)\times \normal{g_j}(\zvec)}{\zvec}\\
P(\rvec_i,\rvec_j\mid H_{22}) &\propto 1
\end{split}
\end{align}
all of which are in the familiar form of inner products between normalized meta-embeddings.
...
\muvec_{a\mid b} &= \muvec_a + \Sigmamat_{ab}\Sigmamat_{bb}^{-1}(\xvec_b - \muvec_b)
\end{align} 
where
\begin{align}
...
% No longer necessary if using package booktabs.
%\def\sp_hline{%
%\noalign{\vskip 0.5mm}%
%\hline%
%\noalign{\vskip 0.5mm}%
