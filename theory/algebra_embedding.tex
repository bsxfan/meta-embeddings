...
\emph{Embeddings} are familiar in modern machine learning and more specifically in speech and speaker recognition. Word embeddings\footnote{\url{en.wikipedia.org/wiki/Word_embedding}} extracted by neural nets were already proposed in 2000 by Bengio~\cite{Bengio_word_embedding}. Now embeddings are used more generally, for example in state-of-the-art face recognition, e.g.\ Facenet~\cite{Facenet}. In Interspeech 2017, eighteen papers have the word `embedding' in the title.\footnote{\url{www.interspeech2017.org/program/technical-program/}.} In speaker recognition and spoken language recognition, we have been using i-vectors, a precursor to embeddings, for almost a decade~\cite{ivector-Brighton,ivec,BUT_ivector_language}. More general embeddings are now appearing in speaker recognition, e.g. in~\cite{Voxceleb} and in David Snyder's work~\cite{end2end,DSIS17} and also in language recognition, e.g.~\cite{LIMSI_Language_embedding}.  
...
\noindent
All of these examples are scored using likelihood ratios of the form~\eqref{eq:generalLR} and are therefore dependent on the data only via the speaker identity likelihood functions. 
...
Finally, we equip	our space with another operator, the \emph{elementwise product} (Hadamard product). Since this product is associative, we write this product using juxtaposition and define it as:
\begin{align}
...
\section{Normalized meta-embedding}
\def\normal#1{\overline{#1}}
...
\section{Notations for inner product and expectation}
...
\section{Note on meta-embeddings as random variables}
\def\Aset{\mathcal{A}}
This note is of theoretical interest and is not essential to understanding the rest of the document. 

Let $(\Zset,\Aset,\pi)$ be a probability space, with $\Zset$ as defined above, the space where the identity variables live; and where $\Aset$ is a suitable sigma-algebra, consisting of subsets of $\Zset$. Let $\pi$, the prior on the identity variable, $z\in\Zset$, be in the form of a probability measure, $\pi:\Aset\to[0,1]$. Let our meta-embeddings be measurable functions from $\Zset$ to $\R$. Under these regularity conditions, our meta-embeddings can be viewed as \emph{random variables}~\cite{Billingsley}. Informally, when $f_j$ is a meta-embedding and if we sample $z\sim\pi$, then the likelihood $f_j(z)$ is a real-valued random variable. 

The interested reader is encouraged to read both~\cite{random} and~\cite{ouwehand} where it is explained how expectations of products of random variables form well-defined inner products. Further reading may include appendix B of~\cite{Bigoni} and references therein.
