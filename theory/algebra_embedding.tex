...
\end{split}
\end{align} 
We can simplify $\tilde\Gmat_k$ by applying the Woodbury identity:\footnote{\url{http://en.wikipedia.org/wiki/Woodbury_matrix_identity}} 
\begin{align}
\tilde\Gmat_k &= \Wmat_k-\Wmat_k\Fmat_k(\Imat+\Bmat_k)^{-1}\Fmat_k'\Wmat_k
= (\Wmat^{-1}+\Fmat\Fmat')^{-1}
\end{align}
from which we can see, $\tilde\Gmat_k^{-1}$ is the variance of $\rvec$, which results if we sample: 
\begin{align*}
...
plays a similar role to $\Gmat$. The (almost ancillary) statistic, $\rvec_j'\tilde\Gmat\rvec_j$ effects a data-dependent modulation of the mixture weights. In fact, if we normalize the modulated weights, they turn out to be the posterior distribution for the component $k$ that was responsible for generating $\rvec_j$:
\begin{align}
P(k\mid\rvec_j) &\propto w_k \sqrt{\frac{\detm{\Wmat_k}}{\detm{\Imat+\Bmat_k}}}
