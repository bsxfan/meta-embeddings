...
\noindent \textbf{In summary:} In each of these simple cases, the likelihood-ratio is most compactly represented as an \emph{inner product between normalized meta-embeddings}. 

\subsection{The general case}
%$$\includegraphics[width=15cm]{lattice_of_partitions}$$
If we consider the most general case of LRs of the form~\eqref{eq:generalLR}, we find that there are cases that cannot be written as a single inner product between normalized meta-embeddings. But we show that they \emph{can} however be written as products and ratios of such inner products. 

We start with an example. Consider a set of four recordings, $\Rset=\{r_1,r_2,r_3,r_4\}$ with associated meta-embeddings, $\{f_1,f_2,f_3,f_4\}$ and consider the two hypotheses: $A:\{1\},\{2,4\},\{3\}$ and $B:\{1,3\},\{2\},\{4\}$. The LR is:
\begin{align}
\begin{split}
\frac{P(\Rset\mid A)}{P(\Rset\mid B)} &= \frac{\expp{f_1}\expp{f_2f_4}\expp{f_3}}{\expp{f_1f_3}\expp{f_2}\expp{f_4}}\\
&= \frac{\expp{f_1}\expp{f_2f_4}\expp{f_3}}{\expp{f_1f_3}\expp{f_2f_4}}\,
\frac{\expp{f_1f_3}\expp{f_2f_4}}{\expp{f_1f_3}\expp{f_2}\expp{f_4}} \\
&= \frac{\expp{f_1}\expp{f_3}}{\expp{f_1f_3}}\,
\frac{\expp{f_2f_4}}{\expp{f_2}\expp{f_4}} \\
&= \frac{\dot{\normal{f_2}}{\normal{f_4}}}{\dot{\normal{f_1}}{\normal{f_3}}}
\end{split}
\end{align} 
We have achieved this re-arrangement by observing that we can get from $A$ to $B$ via the auxiliary hypothesis $C:\{1,3\},\{2,4\}$, where $C$ is reached from $A$ by joining $\{1\}\{3\}$ and $B$ is reached from $C$ by splitting $\{2,4\}$. The join contributes an inner product factor below the line and the split contributes another above the line.

In general, the partitions of a set form a \emph{partial order}, where $C\ge A$ is defined by $C$ \emph{is coarser than} $A$. In our example, we have both $C\ge A$ and $C\ge B$, but $A$ and $B$ are \emph{not directly comparable}. (If two hypotheses are directly comparable---they differ by a single split or join, then their LR can be expressed as a single inner product between normalized meta-embeddings, as we have seen in section~\ref{sec:simpleLR}.) 

More can be said about this partial order. It is in fact a \emph{lattice}, where every pair of partitions has a supremum (least upper bound) and an infimum (greatest lower bound). To form the LR between any pair of embeddings, we can choose to traverse the lattice between them either via the supremum, or the infimum, while every step (split or join) contributes a factor to assemble the required LR. In our example we chose the former, $C=\sup(A,B)$. It is left as an exercise to the reader to verify that traversing via $\inf(A,B)$ gives the same result.
