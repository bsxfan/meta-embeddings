...
Let $\Lset:\Sset_1,\Sset_2,\ldots,\Sset_k$ be a $k$-speaker partition of the set of $n$ recordings, expressed as before in terms of index subsets, $\Sset_j\subseteq\{1,\ldots,n\}$. The CRP probability distribution can be defined as:  
\begin{align}
\label{eq:CRPdef}
...
\end{cases}
\end{align}
where $\lvert\cdot\rvert$ denotes set size and where $\Li:\Sset'_1,\Sset'_2,\ldots,\Sset'_{k'}$ is the partition that results by removing recording index $i$ from $\Lset$. That is, $\Li$ is obtained from $\Lset$ by removing $i$ from the subset where it occurs. If the modified subset remains non-empty, then $k'=k$, else that subset is removed, so that $k'=k-1$. In the trivial case when $n=1$ and $\Lset:\Sset_1=\{1\}$, we allow $\Li$ to be empty, with $k'=0$.
...
Let $\Lset_0$ represent some convenient, fixed partition hypothesis---for example the finest, or the coarsest partition.\footnote{The finest partition is the the hypothesis that $\Rset=\{r_1,\ldots,r_n\}$ was spoken $n$ different speakers, while te coarsest partition has a single speaker.} The partition posterior can be expressed in terms of prior and likelihoods, or likelihood-ratios, as follows:
\begin{align}
\begin{split}
...
Let us return to the intractable MCL objective of section~\ref{sec:MCL} and play around with the composite score idea to find a tractable alternative. If MCL is applied to multiple small data sets rather than one big one, then we can obtain a tractable criterion. Can we combine MCL applied to small subsets of the data, such that it forms a proper composite score? 

Let us assemble our composite score using our favourite log score as building block:
\begin{align}
\label{eq:df1}
S_C(\theta,\Lset) &= \sum_{i=1}^N -\log P(\Lset^{(i)}\mid\Lset^{[i]},\Rset,\theta) 
\end{align} 
Now define $\Lset^{[i]}$ as follows. Let $\Lset^{[i]}:\Iset_{i1},\Iset_{i2},\ldots,\Iset_{im_i}$ be a partition of $\{1,\ldots,n\}$. However, we let each $\Lset^{[i]}$ be strictly coarser than $\Lset$ (the true partition w.r.t. speaker). That is, some or all of the index subsets are the unions of more than one speaker set. The important property of this construction is that for a given $i$, any pair of distinct subsets do not have any speakers in common. Now let $\Lset^{(i)}$ denote the \emph{complement} of the information in $\Lset^{[i]}$, such that combining $\Lset^{[i]}$ and $\Lset^{(i)}$ recovers $\Lset$. We can now further decompose~\eqref{eq:df1} as follows:
\begin{align}
\label{eq:df2}
\begin{split}
S_C(\theta,\Lset) &= \sum_{i=1}^N -\log P(\Lset^{(i)}\mid\Lset^{[i]},\Rset,\theta) \\
...
When $N=1$, we still have a proper scoring rule, but when $N=1$, the trained system never gets to demonstrate that it can differentiate between speakers in different subsets. It would be better to use a smallish $N>1$, or otherwise in a stochastic training setting, we could repeatedly generate $\Lset^{[i]}$ at random.


\subsection{The dual}
In a dual recipe, we can exchange the roles of $\Lset^{(i)}$ and $\Lset^{[i]}$. We can let the conditioning partitions, the $\Lset^{[i]}$, be strictly \emph{finer} than $\Lset$. Again, $\Lset^{(i)}$ is defined as the complement, so that for every $i$, combining $\Lset^{[i]}$ and $\Lset^{(i)}$ recovers $\Lset$. Every subset of $\Lset^{[i]}$ has exactly one speaker, but multiple subsets may index recordings of the same speaker.  
