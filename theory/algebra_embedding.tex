...
\begin{align}
\tilde P(\Lset\mid\Rset,\theta) \propto P(\Lset\mid\Rset,\theta) 
\end{align}
where the constant of proportionality is dependent on $\theta$ and $\Rset$, but not on $\Lset$. In CD we do approximate gradient ascent on the MCL objective, $\log P(\Lset^*\mid\Rset,\theta)$, where $\Lset^*$ is the true partition of $\Rset$ w.r.t.\ speaker. We make use of the fact that the required gradient can be expressed as an expectation w.r.t.\ the posterior:
\begin{align}
\begin{split}
\nabla_\theta \log P(\Lset^*\mid\Rset,\theta) &= \nabla_\theta \log\tilde P(\Lset^*\mid\Rset,\theta)
- \expv{\log\tilde P(\Lset\mid\Rset,\theta)}{P(\Lset\mid\Rset,\theta)} \\
&\approx \nabla_\theta \log\tilde P(\Lset^*\mid\Rset,\theta)
- \frac1K\sum_{\Lset} \log\tilde P(\Lset\mid\Rset,\theta)\end{split}
\end{align}
...
\end{split}
\end{align}  
where as above, $\Lset_0$ is a convenient, fixed partition. The first RHS factor (prior) is given by~\eqref{eq:CRP_recur}, while the second (likelihood-ratio) can be computed with the methods of section~\ref{sec:generalLR}.
