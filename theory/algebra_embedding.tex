...
\end{split}
\end{align} 
where the above equations implicitly define natural means $\avec,\bvec$, precision blocks $\Amat,\Bmat,\Dmat$ and covariance blocks $\Qmat,\Rmat,\Smat$. Block matrix inversion gives:
\begin{align}
...
\Bigr)
\end{align}
By symmetry, the other marginal, $h_k(\ellvec)$, can be found using the same formula, but exchanging the roles of $\Vmat$ and $\Lmat$. The normalization constants can be recovered using~\eqref{eq:GMElogex}.
