...
P(\rvec\mid\zvec_i) &= \TD(\rvec\mid\Fmat\zvec_i,\Imat,\nu) \\
...
\begin{align}
\Bmat &= \Fmat'\Fmat,& \muvec &= \Bmat^{-1}\Fmat'\rvec& \text{and} && \Gmat &= \Imat-\Fmat\Bmat^{-1}\Fmat' 
\end{align}
...
Let's continue to finally assemble our t-distribution likelihood (now again with subscript $j$):
\begin{align}
\begin{split}
...
\begin{itemize}
  ...
  \end{align}
	...
	\begin{align}
	\label{eq:BGWmu}
	\Bmat&=\Fmat'\Wmat\Fmat, & \Gmat&=\Wmat-\Wmat\Fmat'\Bmat^{-1}\Fmat'\Wmat, &\muvec_j&=\Bmat^{-1}\Fmat'\Wmat\rvec_j 
	\end{align}
	Although $\Gmat$ is no longer idempotent in this case, we still have $\Gmat\Fmat\zvec=\nulvec$ and $E_j=\rvec'_j\Gmat\rvec_j$ fulfils the same function as before. Nothing essential has changed with this generalization, because a fixed, invertible linear transform of the data can recover $\Wmat=\Imat$.
%
...
First, let us rewrite the meta-embedding (likelihood function) in a more convenient form:
\begin{align}
f_j(\zvec) &\propto \TD(\zvec\mid \muvec_j,(\beta_j\Bmat)^{-1},\nu)
\end{align} 
where we use the generalization~\eqref{eq:BGWmu} and where have defined:
\begin{align}
\beta_j &= \frac{\nu}{\nu+E_j}
\end{align}
%\avec_j &= \beta_j\Bmat\muvec_j = \beta_j\Fmat'\Wmat\rvec_j
We can rewrite the t-distribution again as a continuous scale mixture of Gaussians and then approximate it with some quadrature recipe as a finite mixture:
\begin{align}
f_j(\zvec) &\propto \int_0^{\infty} \ND\bigl(\zvec\mid \muvec_j,(\frac{\lambda\beta_j}{\nu}\Bmat)^{-1}\bigr)\chi^2_\nu(\lambda)\,d\lambda\\
&\approx\sum_{k=1}^K w_k\, \ND\bigl(\zvec\mid \muvec_j,(\alpha_k\beta_j\Bmat)^{-1}\bigr)
\end{align} 
where $\alpha_k=\frac{\lambda_k}{\nu}$ and $w_k>0$. In the simplest case, we can even choose $K=1$ and use a single Gaussian, with a suitable variance dilation, $\alpha_1<1$ to compensate for the heavy-tailed nature. In our notation we have assumed that the $\{w_k,\alpha_k\}_{k=1}^K$ are independent of the data. It would also be possible to make it data-dependent, in which case variational Bayes might be helpful to determine the values. 

Now given this mixture form, when we compute an expectation of the form $\expp{f_if_j}$, we have to do $K^2$ Gaussian calculations of the form~\eqref{eq:LRMVG}.

TBD: Compute approximate log LR via 2nd-order Taylor series expansion, to give a qaudratic recipe, similar to PLDA scoring. The variables should be $[\avec_i,\beta_i,\avec_j,\beta_j]$, or maybe $[\avec_i,\log\beta_i,\avec_j,\log\beta_j]$ and the expansion should be relative to a fixed point, e.g. all zeros (in the 2nd parametrization). 
