...
\norm{f+g}^2 &= \dot{f+g}{f+g} = \norm{f}^2 +\norm{g}^2 +2\dot{f}{g}
\end{align}
Addition of~\eqref{eq:sqdist} and~\eqref{eq:sumnorm} give the \emph{parallelogram law}:
\begin{align}
\label{eq:parlaw}
\norm{f+g}^2 + \norm{f-g}^2&= 2\norm{f}^2 +2\norm{g}^2
\end{align}
...
Having defined angle, we can also talk about pairs orthogonal vectors, for which the inner product is zero and which have an angle of $\uppi/2$ between them. For orthogonal vectors, with $\dot{f}{g}=0$, we can use~\eqref{eq:sqdist} to derive the \emph{theorem of Pythagoras}:
\begin{align}
\label{eq:Pythagoras}
\norm{f-g}^2 &= \norm{f}^2 +\norm{g}^2 
\end{align} 
...
The following properties of normalized meta-embeddings are easily shown. Be definition, we have that the length of the projection of any normalized meta-embedding on $\onevec$ is unity:
\begin{align}
\label{eq:LR1}
\dot{\normal{f}}{\onevec} = 1
\end{align}
This gives that differences between normalized meta-embeddings live in a hyperplane perpendicular to $\onevec$: 
\begin{align}
\dot{\normal{f}-\normal{g}}{\onevec} = 0
\end{align}
...
We are now ready to demonstrate the utility of our operations on meta-embeddings and show how they can be used to express the LR formulas of chapter~\ref{chap:fp}. 

We start with \emph{simple LRs}, which can be used to compare any two hypotheses for the partitioning of a set of recordings according to speaker, where the partitions differ by splitting or combining a single subset. Then we treat the general case, to compute LRs to compare any arbitrary pair of hypotheses. The chapter concludes with a graphical example to aid understanding.
...
In example 1 we traversed via $C=\sup(A,B)$. If we instead traverse via $1|2|3|4=\inf(A,B)$, we get the \emph{same} decomposition. This is however not true in general---different paths can give different decompositions of the LR. This is shown in the next example.\\
