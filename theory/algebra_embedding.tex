...
In JPLDA, we shall be working with model configurations where every observation, $\rvec_k$, is dependent on exactly one hidden speaker factor and exactly one hidden language factor. (The converse is not true---multiple observations may depend on a given hidden factor.) In graphical model notation, each $\rvec_k$ has exactly one $\zvec$ parent and exactly one $\ellvec$ parent. (Conversely, hidden factors may have multiple children.) If we consider a single observation at a time, there is no ambiguity about which hidden factors are its parents, so that in the notation for marginal and joint likelihoods and posteriors of the forms, $P(\zvec,\ellvec\mid\rvec_k)$, $P(\zvec\mid\rvec_k)$, $P(\rvec_k\mid\zvec,\ellvec)$, $P(\rvec_k\mid\ellvec)$, etc., it is clear which $\zvec$  and $\ellvec$ variables are being referred to. Indeed, notice that when no other observations are given, then dependency is blocked between $\rvec_k$ and all other hidden variables, except its parents.
...
Having clarified that in likelihoods and posteriors conditioned on a single observation, the relevant hidden variables are its two parents, we express Bayes' rule as:
\begin{align}
P(\zvec,\ellvec\mid\rvec_k) &= \frac{P(\ellvec)P(\zvec)P(\rvec_k\mid\zvec,\ellvec)}{\expv{P(\rvec_k\mid\zvec',\ellvec')}{\zvec',\ellvec'}}
\end{align}
where it is understood that the expectation in the denominator is w.r.t. the factorial prior, $P(\ellvec)P(\zvec)$. In a bivariate generalization of our previous definition, we can now define the \emph{normalized bivariate meta-embedding} as:
\begin{align}
\label{eq:norm_fk}
