...
\begin{align}
\label{eq:TMEvsGME}
f_j{\zvec} &\propto \TD(\zvec\mid \mvec_j,\beta_j\Emat,\nu')
\approx \ND\bigl(\zvec\mid\mvec_j,(\beta_j\Emat)^{-1}\bigr)
\end{align}
Nevertheless, the almost-Gaussian TME is \emph{substantially} different from the GME given by Gaussian PLDA. In Gaussian PLDA (with $\nu\to\infty$), the meta-embedding has constant precision, $\Emat$, while in the case of heavy-tailed noise, the precision is modulated by the data-dependent factor, $\frac{\nu'}{\nu+q_j}$.   

Let us examine $q_j$ in more detail, which is very interesting indeed. It turns out to be an \emph{ancillary statistic}---it is \emph{independent} of $\zvec$, because $\Gmat\Fmat=\nulvec$ and so: 
\begin{align}
...
We are interested in matrix factorizations of matrices of the form $\Imat+\beta\Emat$, where $\beta$ varies but $\Emat$ remains fixed. It turns out we do not have to redo the matrix factorization for every $\beta$. Since $\Emat$ is symmetric positive semi-definite, we can factorize it as:
\begin{align}
...
\begin{align}
\Imat+\beta\Emat &= \Vmat(\Imat+\beta\Lambdamat)\Vmat'
\end{align}
for any $\beta\ge0$, the matrix of interest has a factorization of the same form, where we can re-use $\Vmat$. In short, the eigenvectors remain fixed, while if $\lambda$ is an eigenvalue of $\Emat$, then the corresponding eigenvalue of $\Imat+\beta\Emat$ is $1+\beta\lambda$.
...
Let $f_j(\zvec)=\exp[\zvec'\avec_j-\beta_j\zvec'\Emat\zvec]$. Pooling is done by simple addition of the $\avec$ and $\beta$ parameters and gives a representation in the same form. We can therefore do any LR computation by using identical building blocks, of the form 
\begin{align}
F(\avec_i,\avec_j,\beta_i,\beta_j) = \log\dotn{f_i}{f_j}
\end{align}
Let us approximate this using a variant of the symmetrical Taylor series approximation about some fixed value $\beta_0>0$. Letting $\delta_i=\beta_i-\beta_0$ and $\delta_j=\beta_j-\beta_0$ and $F_{ij}=F(\avec_i,\avec_j,\beta_0,\beta_0)$ we do:
\begin{align}
...
\begin{align}
f_{ij}(\zvec) &= \ND\bigr(\zvec\mid\mvec_{ij},(\beta_{ij}\Emat)^{-1}\bigl)
\end{align}  
where by~\eqref{eq:mu_and_E}:
\begin{align}
\mvec_{ij} &= \Emat^{-1}\Fmat'\Wmat\rvec_{ij}\,,&\beta_{ij}&=\frac{\nu+D-d}{\nu+E_{ij}} &\text{and} &&
...
where
\begin{align}
\bar\Emat_i &= \Imat+\sum_j \beta_{ij} \Emat
\end{align}
and
\begin{align}
\begin{split}
\hat\zvec_i &= \bar\Emat_i^{-1}\Emat \sum_j \beta_{ij} \mvec_{ij} = \bar\Emat_i^{-1}\Emat\sum_j \beta_j \mvec_{ij}
= \bar\Emat^{-1}\Emat\Fmat'\Wmat\sum_j  \beta_j\rvec_{ij}
\end{split}
\end{align}
...
\begin{align}
g_j(\zvec) &= \exp[\avec_j'\zvec-\frac{\beta_j}{2}\zvec'\Emat\zvec]
\end{align}
...
&= \avec_j'\hat\zvec_i -\frac{\beta_j}{2}\hat\zvec_i'\Emat\hat\zvec_i -\frac{1}{2}\avec_j(\Imat+\beta_j\Emat)^{-1}\avec_j +\frac12\log\detm{\Imat+\beta_j\Emat}
\end{split}
\end{align}
...
\log L_{ij} &\approx \avec_j'\hat\zvec_i -\frac{\beta_j}{2}\hat\zvec_i'\Emat\hat\zvec_i + \const
\end{align}
This can be arranged into a $(d+1)$-dimensional dot-product for computational convenience. 

Intuitively it would be satisfying for the the score to be modulated by $\beta_j$, so that score magnitudes become smaller for noisier data. This is true for the second term in~\eqref{eq:TPLDA_delta_score}, but what about the first term? We can make the the dependence on $\beta_j$ clear by considering $\muvec_j=(\Imat+\beta_j\Emat)^{-1}\avec_j$, which is the mean of the Gaussian approximation to $P(\zvec\mid \rvec_j,\pi)$. Recall that similarly, $\hat\zvec_i$ is the mean of the Gaussian approximation to $P(\zvec\mid\Rset_i,\pi)$. The first term can now be rewritten as:
$$\avec_j'\hat\zvec_i = \muvec'_i(\Imat+\beta_j\Emat)\hat\zvec_i$$
...
\begin{itemize}
  \item Require: 
	...
	\begin{itemize}
	   ...
	   \item Find all recordings of those speakers and extract their meta-embeddings, where the extractor is parametrized by $\theta$. If there are more than $t$ of these meta-embeddings, pool some of them, respecting speaker purity, until the number of (pooled/unpooled) meta-embeddings is $k$, such that $s\le k \le t$. Denote this set of $k$ meta-embeddings as $\Fset^{[i]}_\theta$ and their true partition w.r.t.\ speaker as $\Lset^{(i)}\in\Pset_k$.
