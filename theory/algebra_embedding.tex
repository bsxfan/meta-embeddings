...
LRs between TMEs are not available in closed form. However, as we found above, when $(D-d)\gg1$, the TME is very close to a GME, even though the noise may be very heavy tailed, with a small $\nu$. Let us proceed with the Gaussian approximation. As we already know, GME LRs are tractable, but slower than quadratic GPLDA scoring. We examine the Taylor series approximation for this case in more detail.
...
Other variants are possible, for example we could let $\beta_i = e^{\gamma_i}$ and do the expansion in terms of $\gamma_i$ instead.
