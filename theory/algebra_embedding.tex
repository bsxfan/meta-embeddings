...
A similar experiment, now with $5000$ synthetic recordings and a prior expectation of $500$ speakers, shows that the overtraining effect is less for bigger data sets:
$$
\begin{tabular}{l l | r r r}
  &  & $S_\Psi$ & BXE & EER \\
\hline
...
The DET-plot is a trade-off of miss rate against false-alarm rate (on a probit scale) as the score decision threshold is varied over the whole real line. It is a calibration-insensitive analysis of the decision-making ability of binary classifier scores. This plot shows that the two systems are essentially equivalent at all operating points and that the overtraining problem has virtually disappeared for the larger training set.\footnote{We ascribe the unfamiliar, steep angle of the DET plot to the use of low-dimensional synthetic data.}
