...
The optimizer could be a full batch optimizer such as L-BFGS, or some variant of stochastic gradient descent, with mini-batches. The choice of optimizer is also to some extent independent of the other choices, although large optimization problems will tend to favour stochastic mini-batch methods. Moreover, as mentioned in chapter~\ref{chap:approx}, approximate stochastic expectations can play well with stochastic optimization.

\section{Proper scoring rules}
\def\Dset{\mathcal{D}}
In the introduction, we have already mentioned that since meta-embeddings quantify uncertainty, we need objective functions that properly evaluate the goodness of the uncertainty. The goodness of uncertainty expressed in a probabilistic framework, as we do here, can be evaluated via \emph{proper scoring rules}~\cite{Gneiting_Raftery_PSR}. If a proper scoring rule is used as training criterion, it is known as \emph{minimum scoring rule inference}~\cite{Dawid_MinScoringRuleInference}. Well known training criteria, such as maximum likelihood, maximum conditional likelihood and minimum cross-entropy are all special cases of minimum scoring rule inference.
...
\end{align}
In other words, the expected value, w.r.t.\ $P$ is minimized at $Q=P$. The canonical proper scoring rule is the \emph{logarithmic scoring rule}, defined as:
\begin{align}
S(Q,e) = -\log Q(e)
\end{align}
With the logarithmic rule (depending on the context), minimum scoring rule inference becomes maximum likelihood, maximum conditional likelihood, or minimum cross-entropy. Here we are interested in discriminative training and therefore in maximum conditional likelihood (MCL), which we will discuss in the next section. Unfortunately, for our training problem, MCL is intractable, so we need to find alternatives, which we will construct using scoring rules other than the logarithmic one.

The other way to define proper scoring rules works like this. Let $Q$ be a probability distribution to predict an event $e\in\Eset$. Let $d\in\Dset$, be a \emph{decision} that is to be made when $Q$ is given while $e$ is still unknown. Let $C(d,e)$ be a real-valued function, known as \emph{cost function} that quantifies the goodness of decision $d$, when afterwards it becomes known that the value of the event is $e$. Let $d^*_Q\in\Dset$ be a \emph{Bayes decision}, with the property:
\begin{align}
...
S(Q,e) &= C(d^*_Q,e)
\end{align}
is a proper scoring rule. It is easy to check that~\eqref{eq:psr2} implies~\eqref{eq:psr1}, but the converse is also true. If we let $\Dset=\Eset$, then the decision can be seen as an attempt to infer the best (lowest expected cost) prediction for $e$. But $\Bset$ can be much more general, so that proper scoring rules are also very flexible.   


\section{The intractable MCL objective}
In supervised training for generative models, \emph{maximum likelihood} (ML) is the natural training criterion: 
...
$$\argmax_\theta P(\text{labels}\mid\text{data},\theta)=\argmax_\theta \sum_i \log P(\text{label}_i\mid\text{datum}_i,\theta)$$
...
\end{align}
If we are given a meta-embedding for every $r_j$, and if arbitrary pooling and expectations (or inner products) are tractable, then we also have that $L(h,h^*;\theta)$ is tractable for any $h,h^*$. The problem is that the number of terms in the denominator (the Bell number) is hopelessly intractable, except for very small $n$. (A speaker recognizer is typically trained with $n$ in the thousands.)
