...
\chapter{Toy example with heavy-tailed noise}
...
The likelihood, $P(\rvec_j\mid\zvec_i)$ is parametrized by a rectangular, $D$-by-$d$ \emph{factor loading matrix}, $\Fmat$, and the \emph{degrees of freedom}, an integer, $\nu\ge1$. For speaker $i$, multiple observations, $\rvec_j\in\R^D$, are produced by the addition of independently drawn, heavy-tailed noise:
\begin{align}
\rvec_j &= \Fmat\zvec_i + \etavec_j 
\end{align}
The heavy-tailed noise is effectively Gaussian with variable precision, $\lambda_j$, sampled from the chi-squared distribution, with degrees of freedom $\nu$:   
\begin{align}
\etavec_j&\sim \ND(\nulvec,\tfrac{\nu}{\lambda_j}\Imat) &\text{and} && \lambda_j &\sim \chi_\nu^2
\end{align}
The expected value\footnote{By Jensen's inequality, the expected value of the \emph{variance} scaling factor, is greater than 1. For $\nu>2$, it is $\expp{\frac{\nu}{\lambda_j}}=\frac{\nu}{\nu-2}$.} of the precision scaling factor is $\expp{\frac{\lambda_j}{\nu}}=1$. Marginalizing out the hidden $\lambda_j$ gives the likelihood, which is a t-distribution:\footnote{\url{http://en.wikipedia.org/wiki/Multivariate_t-distribution}}
\begin{align}
...
\end{align}
By examining its definition above, you can check that $\Gmat$ is an \emph{idempotent projection matrix}, which removes the influence of $\zvec$ from $\rvec$. That is, $\Gmat\rvec$ is the projection of $\rvec$ onto the complement of the subspace spanned by $\Fmat$. Intuitively, $E_{\rvec_j}/(D-d)$ estimates the hidden noise variance $\lambda_j^{-1}$. This statistic is called \emph{ancillary}, because it is independent of $\zvec$, which we are trying to infer, yet it is still relevant to that inference, as we shall see. Notice that this statistic only works for $D> d$. If we have $d=D$, and $\Fmat$ is invertible, then $\Gmat=\nulvec$ and $E=0$.
...
Let us examine $E_j$ in more detail. Since it is computed from heavy-tailed variables, it is also going to be heavy-tailed. Its scale is proportional to $D-d$ and will (as indeed intuition suggests) have a greater effect for larger dimensionality differences. Its scale is however independent of $\nu$, so that for large $\nu$ (more Gaussian noise), the factor $\frac{\nu}{\nu+E_j}$ will tend to unity---this also agrees with intuition: since the noise precision scaling factor, $\frac{\lambda_j}{/nu}$, will be almost constant for large $\nu$. 
