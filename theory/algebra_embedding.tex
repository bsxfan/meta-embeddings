...
\begin{align}
P(\Rset_i\mid\zvec,\lambdavec) &= \prod_{j=1}^{N_i} \ND(\rvec_{ij}\mid \Fmat\zvec,(\lambda\Wmat)^{-1})
\end{align}
...
\begin{align}
\begin{split}
\log Q_{ij}(\lambda) &= \log \GD(\lambda\mid\alpha,\beta) + \expv{\log \ND(\rvec_{ij}\mid\lambda,\zvec)}{Q_i(\zvec)} + \const \\
