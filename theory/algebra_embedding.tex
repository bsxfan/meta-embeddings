...
We are interested in expectations of the form $\expv{\prod_{i=1}^m f_i}{\pi}$, which we rewrite via Parseval's and convolution theorems as:
\begin{align}
...
\begin{align}
\label{eq:FT}
\FT{f}(\zetavec) &= \tilde f(\zetavec) = \int_{\R^d} f(\zvec)e^{-2\pi i\zvec'\zetavec}\,d\zvec
\end{align}
where $\zvec'\zetavec$ denotes dot product. The inverse transform (IFT) is:
\begin{align}
\IFT{\tilde f}(\zvec) &= f(\zvec) = \int_{\R^d} \tilde f(\zetavec)e^{2\pi i\zvec'\zetavec}\,d\zetavec
\end{align}
...
\begin{align}
\label{eq:ftdelta}
\FT[\zvec]{\delta(\zvec-\yvec)}(\zetavec) &= \int_{\R^d} \delta(\zvec-\yvec)e^{-2\pi\zvec'\zetavec}\,d\zvec
= e^{-2\pi i\yvec'\zetavec}
\end{align}
...
\begin{align}
\label{eq:iftdelta}
\delta(\zvec-\yvec) &= \int_{\R^d} e^{-2\pi i\yvec'\zetavec} e^{2\pi i \zvec'\zetavec} \,d\zetavec
= \int_{\R^d} e^{2\pi i(\zvec-\yvec)'\zetavec} \,d\zetavec
\end{align}


\section{Shifting property}
...
\begin{align}
\begin{split}
\FT[\zvec]{f(\zvec-\yvec)}(\zetavec) &= \int_{\R^d} f(\zvec-\yvec)e^{-2\pi i\zvec'\zetavec}\,d\zvec\\
&= \int_{\R^d} f(\rvec)e^{-2\pi i(\rvec+\yvec)'\zetavec}\,d\rvec \\
&= e^{-2\pi i\yvec'\zetavec}\int_{\R^d} f(\rvec)e^{-2\pi i\rvec'\zetavec}\,d\rvec \\
&= e^{-2\pi i\yvec'\zetavec}\tilde f(\zetavec) \\
\end{split}
\end{align}
...
\end{align}
Of course, we have: $\norm{f}=\int_{\R^d}\abs{f(\zvec)}^2\,d\zvec$.\\
...
&= \int_{\R^d} \left(\int_{\R^d}\tilde f(\zetavec)e^{2\pi i \zetavec'\zvec} \,d\zetavec\right) \conj{\left(\int_{\R^d}\tilde g(\yvec)e^{2\pi i \yvec'\zvec} \,d\yvec\right)}\,d\zvec \\
&= \int_{\R^d} \left(\int_{\R^d}\tilde f(\zetavec)e^{2\pi i \zetavec'\zvec} \,d\zetavec\right) \left(\int_{\R^d}\conj{\tilde g(\yvec)}e^{-2\pi i \yvec'\zvec} \,d\yvec\right)\,d\zvec \\
&= \int_{\R^d}\int_{\R^d} \tilde f(\zetavec)\conj{\tilde g(\yvec)} \left(\int_{\R^d} e^{2\pi i(\zetavec-\yvec)'\zvec}\,d\zvec \right) \,d\zetavec\,d\yvec \\
...
\end{align}
Notice that since $\abs{e^{i\tvec'\zvec}}\le1$, we also have $\abs{\phi_p(\tvec)}\le1$. Indeed, the characteristic function always exists. If $P$ has density function, $p(\zvec)$, then density and characteristic function essentially form a Fourier transform pair:
\begin{align}
\phi_P(\tvec) &= \int_{\R^d} e^{i\tvec'\zvec} p(\zvec)\,d\zvec
\end{align}
For convenience in this document, observe that if we let $\tvec=-2\pi\zetavec$, then: 
\begin{align}
\tilde p(\zetavec) = \phi_P(-2\pi\zetavec) &= \int_{\R^d} e^{-2\pi i\zetavec'\zvec} p(\zvec)\,d\zvec = \FT{p}(\zetavec)
\end{align}
We shall work with $\tilde p$, rather than $\phi_P$, while keeping in mind that the one is just a reparametrized version of the other. Indeed, where the distinction is unimportant, we shall also refer to $\tilde p$ as characteristic function. 

The characteristics functions for most standard probability distributions are known~\cite{Oberhettinger}. 

\subsection{Parseval}
How does Parseval's theorem translate in terms of characteristic functions? Let $f,g$ be probability density functions with characteristic functions, $\phi_f$ and $\phi_g$. Then, letting $\tvec=-2\pi\zetavec$ and $d\tvec=(2\pi)^d d\zetavec$
\begin{align}
\begin{split}
\int_{\R^d} f(\zvec)g(\zvec)\,d\zvec &= \int_{\R^d} \tilde f(\zetavec)\conj{\tilde g(\zetavec)}\,d\zetavec \\
&= \int_{\R^d} \phi_f(-2\pi\zetavec)\conj{\phi_g(-2\pi\zetavec)}\,d\zetavec\\
&= \frac1{(2\pi)^d}\int_{\R^d} \phi_f(\tvec)\conj{\phi_g(\tvec)}\,d\tvec\\
\end{split}
\end{align}
...
\begin{align}
\begin{split}
\phi_{\ND}(\tvec;\muvec,\Sigmamat) &= \int_{\R^d} \ND(\zvec\mid \muvec,\Sigmamat) \exp(i\tvec'\zvec) \,d\zvec \\
&= \exp(i\muvec'\tvec-\frac12\tvec'\Sigmamat\tvec)
\end{split}
\end{align}
Notice that for zero mean, $\muvec=\nulvec$, the characteristic function is also an unnormalized zero mean Gaussian, with the roles of covariance and precision interchanged: 
\begin{align}
\begin{split}
\phi_{\ND}(\tvec;\nulvec,\Sigmamat) &= \sqrt{\detm{2\pi\Sigmamat^{-1}}}\ND(\tvec\mid\nulvec,\Sigmamat^{-1}) \\
&= \frac{(2\pi)^{d/2}}{\sqrt{\detm{\Sigmamat}}}\ND(\tvec\mid\nulvec,\Sigmamat^{-1})
\end{split}
\end{align}
As the PDF gets more concentrated, its transform gets more spread out---this inverse scaling is a general property of the Fourier transform. 

Finally, if $\muvec$ is non-zero, then the characteristic function is not real-valued and is therefore not a probability density.
...
\begin{align}
\label{eq:empc}
\tilde p(\zetavec) &= \frac1m\sum_{\ell=1}^m e^{-2\pi i\zetavec'\zvec_\ell}
\end{align}
...
=&\sum_{\ell=1}^m \sum_{j=1}^m \alpha_\ell\conj{\alpha_j} \int_{\R^d}r(\zvec)e^{-2\pi i(\zetavec_\ell-\zetavec_j)'\zvec}\,d\zvec \\
=&\int_{\R^d}r(\zvec)\sum_{\ell=1}^m \sum_{j=1}^m \alpha_\ell e^{-2\pi i\zetavec_\ell'\zvec}\conj{\alpha_j e^{-2\pi i\zetavec_j'\zvec}}\,d\zvec \\
=&\int_{\R^d}r(\zvec)\left\lvert\sum_{\ell=1}^m \alpha_\ell e^{-2\pi i\zetavec_\ell'\zvec}\right\rvert^2\,d\zvec \ge 0\\
\end{split}
\end{align}
