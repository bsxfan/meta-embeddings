...
\draw (top) to (n12_34);
\end{tikzpicture}
\caption[Hasse diagram of the lattice of partitions]{Hasse diagram of the lattice of partitions of a set of 4 recordings, ordered by `refines'. Every downward arc indicates an atomic refinement step, which splits a single subset. Example 1 is highlighted in blue , example 2a in red and 2b in green.}
\label{fig:lattice}
\end{figure}
...
In example 1 we traversed via $C=\sup(A,B)$. If we instead traverse via $1|2|3|4=\inf(A,B)$, we get the \emph{same} answer. This is however not true in general---different paths will give different decompositions of the LR. This is shown in the next example.\\
