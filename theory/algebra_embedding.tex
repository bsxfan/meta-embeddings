...
\subsection{Training}
Let class $i$ have training data, $\Rset_i=\{\rvec_{ij}\}_{j=1}^{N_i}$, represented by the approximate Gaussian meta-embeddings:
\begin{align}
...
\end{align}
we can compute the log-likelihood for each class $i$ as:
\begin{align}
\begin{split}
\log L_{ij} &= \log g_j(\hat\zvec_i) - \log\expp{g_j} \\
...
\begin{align}
\log L_{ij} &= \avec_j'\hat\zvec_i -\frac{\beta_j}{2}\hat\zvec_i'\Bmat\hat\zvec_i + \const
\end{align}
