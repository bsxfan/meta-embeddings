...
\begin{align}
\sum_{s=1}^K\sum_{t=1}^K \exp[\rvec'\Wmat\mvec_s -\frac12\mvec_s\Wmat\mvec_s] 
\exp[- \mvec_s\Wmat\muvec_t] 
\exp[\rvec'\Wmat\mvec_t -\frac12\mvec_t\Wmat\mvec_t]
\end{align}
...
\end{itemize}
In both cases, the matrix can be simplified offline, before runtime, because it is not dependent on $\rvec_j$.
