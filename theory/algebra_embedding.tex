...
The pseudolikelihood computational load scales as $kn$, where $k$ is the number of speakers and $n$ the number of recordings. In this respect it is similar to generative training of PLDA models and similar to discriminative cross-entropy training of multiclass classifiers. It must however be noted that pseudolikelihood may not play well with stochastic minibatch optimizers. The most important condition for stochastic optimization is that the objective function must decompose into a (large)  number of terms. This is true of pseudolikelihood---it has $n$ terms, but unfortunately every term needs access to statistics that are dependent on the parameters being trained and that have been computed over the whole dataset. 
