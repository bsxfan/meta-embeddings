...
The figure below shows some meta-embeddings and the LRs between them, computed using~\eqref{eq:exactMVG} in~\eqref{eq:slrdot}, or \eqref{eq:mlrdot}. Blue, red and green represent raw embeddings, while black represents blue and red pooled, where pooling is done by adding natural parameters ($\avec_\text{black} = \avec_\text{blue} + \avec_\text{red}$, and $\Bmat_\text{black} = \Bmat_\text{blue} + \Bmat_\text{red}$):  
$$
\begin{tikzpicture}[xscale=1,yscale=1]
\draw[rotate around ={   0:( 0.3,   0)},blue] ( 0.3,   0) ellipse [x radius=0.447, y radius=   1];
\draw[rotate around ={  90:(   0,   0)},red] (   0,   0) ellipse [x radius=0.316, y radius=   1];
\draw[rotate around ={   0:(   2,   0)},green] (   2,   0) ellipse [x radius=0.577, y radius=0.577];
\draw[rotate around ={  90:(0.25,   0)},black] (0.25,   0) ellipse [x radius=0.302, y radius=0.408];
\node[ctext] at(6,0) {%
\begin{tabular}{c|r|r|r}
$\dotn{f_i}{f_j}$ & blue & red & black \\
\hline
green & 0.1  & 0.18 &  0.05 \\
blue & & 0.55 &  
\end{tabular}%
...
\end{tikzpicture}
$$
Notice that LR(blue,green) $<$ LR(red,green), even though the blue mean is closer to green than the red mean. This is due to the greater red uncertainty in the horizontal direction. Pooling (black) dramatically reduces the uncertainty about the location of $\zvec$ for the speaker represented by red and blue and consequently increases the certainty that this speaker is not the same as the green speaker: LR(black,green) is smaller than all the other LRs.

The only surprise in this example is perhaps that LR(red,blue) $<1$. Even though the red and blue distribution means are close, the overlap between them is not enough to favour the same-speaker hypothesis. 
