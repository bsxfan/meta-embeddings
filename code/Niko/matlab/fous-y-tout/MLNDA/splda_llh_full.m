function [llh,back] = splda_llh_full(R,labels,F,W)
% Like splda_llh(), but backpropagates into F,W
%
% Inputs:
%   R: D-by-N, i-vectors
%   labels: sparse, logical K-by-N, one hot columns, K speakers, N recordings
    ...
    cholW = chol(W);
    logdetW = 2*log(sum(diag(cholW)));
    
    nE = bsxfun(@times,n,E);
    logdetP = sum(log1p(nE),1);   % logdets of posterior precisions
    
    llh = llh + ndata*logdetW/2 - sum(logdetP,2)/2;
