...
%    fscal = 1/100;      %increase fscal to move speakers apart
    ...
    niters = 1000;
    reg_weight = 100;
    model3 = SGME_train2(Train,ilabels,nu,zdim,reg_weight,niters);
    ...
    if useBOSARIS
        ...
        Scores = model3.score_trials(Enroll1,[],Test);
        [BXE(3,1),minBXE(3,1),EER(3,1)] = calcBXE(Scores,hlabels);
        Scores = model3.score_trials(Enroll2,Flags,Test);
        [BXE(3,2),minBXE(3,2),EER(3,2)] = calcBXE(Scores,hlabels);
        ...
        fprintf('PsL   : single enroll BXE = %g (%g), double enroll BXE = %g (%g)\n',BXE(3,1),minBXE(3,1),BXE(3,2),minBXE(3,2));

        fprintf('oracle: single enroll EER = %g, double enroll EER = %g\n',EER(1,1),EER(1,2));
        fprintf('VB    : single enroll EER = %g, double enroll EER = %g\n',EER(2,1),EER(2,2));
        fprintf('PsL   : single enroll EER = %g, double enroll EER = %g\n',EER(3,1),EER(3,2));
